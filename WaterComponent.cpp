...
WaterComponent::WaterComponent(const glm::vec2 &size, float resolution)
{
	...
	renderContextDeferred = WaterRenderContext::getInstance();
...
void WaterComponent::drawForwardPass()
{
	glEnable(GL_BLEND);
	...
	WaterRenderContext* context = dynamic_cast<WaterRenderContext*>(renderContextDeferred);
